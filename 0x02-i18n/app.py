...
def get_timezone():
    """Retrives time zone from global object
    """
...
def hello_world() -> str:
    ...
    curr_t = datetime.datetime.now(pytz.timezone(user_tz)).strftime(
                          '%b %d, %Y, %I:%M:%S %p')
    return render_template('7-index.html', user_name=user_n, time_zone=curr_t)
